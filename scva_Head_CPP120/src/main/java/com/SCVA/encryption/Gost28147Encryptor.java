...
public class Gost28147Encryptor extends BaseEncryptor {

    public Gost28147Encryptor(String key){
        super(key);
        setKey(key_in_bytes);
    }
    @Override
    public String encrypt(String plain) {
        ...
        DataInputStream ins = new DataInputStream(new ByteArrayInputStream(builder.toString().getBytes()));
        ByteArrayOutputStream os = new ByteArrayOutputStream();
        DataOutputStream dos = new DataOutputStream(os);

        try {
            rpz(Mode.ENCRYPT,dos,ins);
            dos.flush();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private byte gost_key[][] = new byte[8][4];

    private void setKey(byte[] key) {
        for (int i=0;i<8;i++)
            for(int j=0;j<4;j++)
                this.gost_key[i][j] = key[i*4 + j];
    }
    ...
    public void rpz(Mode mode, DataOutputStream dos, DataInputStream dis) throws Exception {
        ...
        while (count != -1) {
            ...
            for (int k = 0; k < 32; k++) {
                byte[] K = Mode.ENCRYPT.equals(mode) ? gost_key[keyMap[k]] : gost_key[keyMap[31 - k]];
