...
void psk::findAFCDirection()
{
	updatePower();
	double lfreq = frequency - sc_bw / 2;
    double hfreq = frequency + sc_bw / 2;
    double lpwr = powerDensity(lfreq, sc_bw / 2);
    double hpwr = powerDensity(hfreq, sc_bw / 2);
    afcDirectionUp = (lpwr < hpwr) ? true : false;
    //__android_log_print(ANDROID_LOG_VERBOSE, "FindAFCDirection", "lpwr: %lf around %lf, hpwr: %lf around %lf => afcDirection: %s", lpwr, lfreq, hpwr, hfreq, afcDirectionUp ? "Up" : "Down");
}
...
//Android: review when have selectable afc
    ...
    while (srchfreq > minfreq) {
        //@! Changed to sc_bw/2
        spwr = powerDensity(srchfreq, sc_bw/2);
        npwr = powerDensity(srchfreq + sc_bw, sc_bw/2) + 1e-10;
    ...
    setPreferenceS("AFREQUENCY", std::to_string((int) frequency));
}
...
//Android: re-enable (delete return below and check for multi-carrier modes) when have selectable afc
	...
	if (fabs(error) < sc_bw ) {
		...
		if ((frequency < getPreferenceI("AFREQUENCY", 1500) - 1) && afcDirectionUp)
			frequency = getPreferenceI("AFREQUENCY", 1500) + 15;
		if ((frequency > getPreferenceI("AFREQUENCY", 1500) + 1) && !afcDirectionUp)
			frequency = getPreferenceI("AFREQUENCY", 1500) - 15;

		if (getPreferenceB("LIMITAFC", true))
		{
			if (frequency < getPreferenceI("AFREQUENCY", 1500) - getPreferenceI("PSKAFCRANGE", 25))
				frequency = getPreferenceI("AFREQUENCY", 1500) - getPreferenceI("PSKAFCRANGE", 25);
			if (frequency > getPreferenceI("AFREQUENCY", 1500) + getPreferenceI("PSKAFCRANGE", 25))
				frequency = getPreferenceI("AFREQUENCY", 1500) + getPreferenceI("PSKAFCRANGE", 25);
		}
...
//Android Always OFF if (!progStatus.afconoff)
	progdefaults.afconoff  = getPreferenceB("AFCONOFF", true);
	if (!progdefaults.afconoff)
		return;
	if (dcd == true || acquire)
		phaseafc();
	else if (lastAFCState)
	{
		lastAFCState = false;
		setPreferenceS("AFREQUENCY", std::to_string((int) frequency));
	}
...
double psk::powerDensity(double f0, double bw) {
    double pwrdensity = 0.0;
    int flower = (int)((f0 - bw/2)),
            fupper = (int)((f0 + bw/2));
    if (flower < 0 || fupper > 2500)
        return 0.0;
    {
        for (int i = flower; i <= fupper; i++)
            pwrdensity += pwr[(int)(i * 2048.0 / RSID_SAMPLE_RATE)];
    }
